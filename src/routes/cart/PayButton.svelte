...
<form method="post" class="inputF" action = "?/pay">
    <div class = "totalT">Total amount to pay: <div class ="price">{totalSum + "$"}</div></div>
    <input name="usercartId" value={value} type="hidden"/>
    <button class = "button"  on:click={ () => console.log("Pay for items")}>
...
.button {
    ...
    border:hidden;
    border-bottom: 1px #d1d1d19a solid;
    border-radius: 10px;
    font-weight: bold;
